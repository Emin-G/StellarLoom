...
module.exports = {
	data: new SlashCommandBuilder()
	    .setName("나이트")
		.setDescription("NightCore 모드를 켜거나 끕니다. 드가자아아아아아아아아아아아아아아아아아아아아아아아아아아아아악"),
	...
	async execute(interaction) {
        ...
        if (!connection[interaction.guild.id].filtersApi) {
            connection[interaction.guild.id].filtersApi = "NIGHT";

            await connection[interaction.guild.id].filters({ timescale: { speed: 1.1, pitch: 1.2, rate: 1 }, equalizer: [ {band: 0, gain: 0.10}, {band: 1, gain: 0.10}, {band: 2, gain: 0.10}, {band: 3, gain: 0.10}, {band: 4, gain: 0.05}, {band: 5, gain: 0}, {band: 6, gain: -0.05}, {band: 7, gain: -0.05}, {band: 8, gain: -0.05}, {band: 9, gain: 0.05}, {band: 10, gain: 0.05}, {band: 11, gain: 0.05}, {band: 12, gain: 0}, {band: 13, gain: 0}, {band: 14, gain: 0} ] });
            
            let rpemb = new EmbedBuilder()
            .setColor("#28edbc")
            .setTitle(":crescent_moon:  **|**  NightCore 모드를 적용중입니다...")
            interaction.reply({ embeds: [rpemb] });

            setTimeout(() => {
                rpemb = new EmbedBuilder()
                .setColor("#28edbc")
                .setTitle(":crescent_moon:  **|**  NightCore!")
                return interaction.editReply({ embeds: [rpemb] });
            }, 5000);
        }

        else if (connection[interaction.guild.id].filtersApi === "NIGHT") {
            ...
            let rpemb = new EmbedBuilder()
            .setColor("#28edbc")
            .setTitle(":sunny:  **|**  NightCore 모드를 해제중입니다...")
            interaction.reply({ embeds: [rpemb] });

            setTimeout(() => {
            rpemb = new EmbedBuilder()
                .setColor("#28edbc")
                .setTitle(":sunny:  **|**  DayCore.")
                return interaction.editReply({ embeds: [rpemb] });
            }, 5000);
        }

        else return embeded.replyEmbed(interaction, ":triangular_flag_on_post:  **|**  BassBoost 기능이 켜져 있습니다.", "BassBoost 기능과 NightCore 모드는 동시에 사용할 수 없습니다.");
